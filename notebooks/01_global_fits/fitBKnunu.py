...
br_exp = flavio.combine_measurements('Rtaul(B->Dlnu)')

def likelihood_global(x, func):
    br_np = flavio.np_prediction('BR(B+->Knunu)', func(x))
    br_np = flavio.np_prediction('BR(B+->Knunu)', func(x))
    return (br_np-br_exp.central_value)**2/br_exp.error_left**2
...
start = 0
print('C1 C3')
with open('likelihoodBKnunu_rotBIII_C1C3.dat', 'at') as f:
...
with open('likelihoodBKnunu_rotBIII_C1beta.dat', 'at') as f:
    for i in range(start, 50*50):
        lg = lh_C1beta(i)
        if i%50 == 49:
            sep = '\n'
...
with open('likelihoodBKnunu_rotBIII_C3beta.dat', 'at') as f:
    for i in range(start, 50*50):
        lg = lh_C3beta(i)
        if i%50 == 49:
            sep = '\n'
